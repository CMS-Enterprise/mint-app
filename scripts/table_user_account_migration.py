...
migration_config = [
    ('Plan_Ops_Eval_and_Learning', 'EASI-2612'),
    # ('Plan_Participants_and_Providers', 'EASI-2613'),
    # ('Plan_Payments', 'EASI-2614'),
    # ('Plan_Beneficiaries', 'EASI-2616'),
    # ('Plan_General_Characteristics', 'EASI-2617'),
]
...
def generate_sql_migration(repo, table, migration_index):
    ...
    migration_sql = generate_sql_migration_content(table)
    if safe_mode:
...
def generate_user_account_table_migration(repo, table, ticket, migration_index):
    create_branch(repo, table, ticket)
    generate_sql_migration(repo, table, migration_index)
    repo.git.checkout('main')
...
def main():
    ...
    for migration in migration_config:
        generate_user_account_table_migration(repo, migration[0], migration[1], migration_index)
