...
func (suite *ResolverSuite) TestErrorIfNotCollaborator() {
	...
	err = errorIfNotCollaborator(plan, suite.testConfigs.Logger, macUser, suite.testConfigs.Store)
	suite.Error(err)
	...
	err = errorIfNotCollaborator(reply, suite.testConfigs.Logger, macUser, suite.testConfigs.Store)
	suite.Error(err)
	...
	err = errorIfNotCollaborator(mtoSol, suite.testConfigs.Logger, basicUserPrincipal, suite.testConfigs.Store)
	suite.NoError(err)

	//10. User is  not collaborator by solutionID
	err = errorIfNotCollaborator(mtoSol, suite.testConfigs.Logger, notCollab, suite.testConfigs.Store)
	suite.Error(err)

	//11. User is not collaborator by solutionID, but is assessment user
	err = errorIfNotCollaborator(mtoSol, suite.testConfigs.Logger, assessment, suite.testConfigs.Store)
	suite.NoError(err)

	//12. Mac Users are NEVER collaborators
	err = errorIfNotCollaborator(mtoSol, suite.testConfigs.Logger, macUser, suite.testConfigs.Store)
	suite.Error(err)
