...
func (suite *ResolverSuite) TestModelPlanUpdate() {
	...
	result, err := ModelPlanUpdate(suite.testConfigs.Context, suite.testConfigs.Logger, plan.ID, changes, suite.testConfigs.Principal, suite.testConfigs.Store, nil, email.AddressBook{}) // update plan with new user "UPDT"
...
func (suite *ResolverSuite) TestModelPlanCollection() {
	...
	result, err := ModelPlanCollection(suite.testConfigs.Context, suite.testConfigs.EChimpS3Client, suite.testConfigs.viperConfig, suite.testConfigs.Logger, clabPrincipal, suite.testConfigs.Store, model.ModelPlanFilterCollabOnly)
	suite.NoError(err)
	suite.NotNil(result)
	suite.Len(result, 1)

	// Assert that CLAB sees all 5 model plans with include all filter
	result, err = ModelPlanCollection(suite.testConfigs.Context, suite.testConfigs.EChimpS3Client, suite.testConfigs.viperConfig, suite.testConfigs.Logger, clabPrincipal, suite.testConfigs.Store, model.ModelPlanFilterIncludeAll)
	suite.NoError(err)
	suite.NotNil(result)
	suite.Len(result, 6)

	// Assert that TEST only sees all 5 model plans with collab only filter (as they're a collaborator on all of them)
	result, err = ModelPlanCollection(suite.testConfigs.Context, suite.testConfigs.EChimpS3Client, suite.testConfigs.viperConfig, suite.testConfigs.Logger, suite.testConfigs.Principal, suite.testConfigs.Store, model.ModelPlanFilterCollabOnly)
	suite.NoError(err)
	suite.NotNil(result)
	suite.Len(result, 6)

	// Assert that TEST sees all 5 model plans with include all filter
	result, err = ModelPlanCollection(suite.testConfigs.Context, suite.testConfigs.EChimpS3Client, suite.testConfigs.viperConfig, suite.testConfigs.Logger, suite.testConfigs.Principal, suite.testConfigs.Store, model.ModelPlanFilterIncludeAll)
	suite.NoError(err)
	suite.NotNil(result)
	suite.Len(result, 6)

	// Assert that TEST sees all model plans with CR / TDLS (ECHIMP)
	result, err = ModelPlanCollection(suite.testConfigs.Context, suite.testConfigs.EChimpS3Client, suite.testConfigs.viperConfig, suite.testConfigs.Logger, suite.testConfigs.Principal, suite.testConfigs.Store, model.ModelPlanFilterWithCrTdls)
	suite.NoError(err)
	suite.NotNil(result)
	...
	suite.Run("Models Approaching Clearance grabs models approaching clearance within 6 months", func() {
		...
		result, err = ModelPlanCollection(suite.testConfigs.Context, suite.testConfigs.EChimpS3Client, suite.testConfigs.viperConfig, suite.testConfigs.Logger, suite.testConfigs.Principal, suite.testConfigs.Store, model.ModelPlanFilterApproachingClearance)
		suite.NoError(err)
