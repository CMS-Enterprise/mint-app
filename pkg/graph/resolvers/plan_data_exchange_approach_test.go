...
type testDataExchangeEvaluateStatusType struct {
	hasChanges          bool
	isSettingToComplete bool
	expectedStatus      models.DataExchangeApproachStatus
}

func (suite *ResolverSuite) TestEvaluateStatus() {
	testCases := []testDataExchangeEvaluateStatusType{
		{
			hasChanges:          true,
			isSettingToComplete: true,
			expectedStatus:      models.DataExchangeApproachStatusCompleted,
		},
		{
			hasChanges:          true,
			isSettingToComplete: false,
			expectedStatus:      models.DataExchangeApproachStatusInProgress,
		},
		{
			hasChanges:          false,
			isSettingToComplete: false,
			expectedStatus:      models.DataExchangeApproachStatusReady,
		},
		{
			hasChanges:          false,
			isSettingToComplete: true,
	...
	for _, testCase := range testCases {
		retStatus := EvaluateStatus(testCase.hasChanges, testCase.isSettingToComplete)
		suite.EqualValues(testCase.expectedStatus, retStatus)
	}
