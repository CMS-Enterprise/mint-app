...
func (suite *ResolverSuite) TestTimelineDateProcessorExtractChangedDates() {

	// Set up some test times
	t1, _ := time.Parse(time.RFC3339, "2024-01-01T00:00:00Z")
	t2, _ := time.Parse(time.RFC3339, "2024-02-02T00:00:00Z")

	// Define a default existing Timeline object
	defaultExisting := &models.Timeline{
		CompleteICIP:            &t1,
		ClearanceStarts:         &t1,
		ClearanceEnds:           &t1,
		Announced:               &t1,
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			dp, _ := NewTimelineDateProcessor(tc.changes, tc.existing)
			changes, _ := dp.ExtractTimelineChangedDates()
