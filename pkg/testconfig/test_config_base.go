...
func GetDefaultTestConfigs() *Base {
	tc := Base{}
	tc.GetDefaults()
	return &tc
}
...
func (config *Base) GenericSetupTests(getTestPrincipal GetTestPrincipalFunction) error {
	...
	princ, err := getTestPrincipal(config.Store, config.UserInfo.Username)
	if err != nil {
		return err
	}
