...
func TranslateAuditJobByID(ctx context.Context, store *storage.Store, logger *zap.Logger, auditID int, queueID uuid.UUID) error {
	...
	_, translateErr := TranslateAudit(ctx, store, logger, auditID)
	if translateErr != nil {
