...
func (suite *WorkerSuite) TestTranslateAuditJob() {
	...
	suite.Run("A queue entry for a duplicate translated audit will not fail the translation queue job", func() {
		...
		_, err = translatedaudit.TranslatedAuditQueueUpdate(suite.testConfigs.Store, logging.NewZapLogger(suite.testConfigs.Logger), entryToTest, suite.testConfigs.Principal.UserAccount.ID)
		suite.NoError(err)
