...
//go:embed SQL/audit_change_collection_by_date.sql
var auditChangeCollectionByDate string
...
func (s *Store) AuditChangeCollectionBydDate(logger *zap.Logger, date time.Time, sortDir models.SortDirection) ([]*models.AuditChange, error) {
	auditChanges := []*models.AuditChange{}
	orderedQuery := auditChangeCollectionByDate
	orderClause := "" //default to ASCENDING
	if sortDir == models.SortDesc {
		orderClause = " ORDER BY 1 DESC"
	}
