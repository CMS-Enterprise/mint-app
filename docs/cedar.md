...
The CEDAR Intake Swagger file requires a bit of preprocessing before code generation. When the Swagger file is updated, put it in `pkg/cedar/intake/cedar_intake.json`, then run `scripts/generate_cedar_intake_client` to preprocess it and regenerate code. See [the intake folder's README](/pkg/shared/cedar/intake/README.md) for more information.

### Connecting to CEDAR when running locally

Make sure the `cedarCoreEnabled` LaunchDarkly flag is set to true. If you're not connected to LaunchDarkly during local development, in [`pkg/cedar/core/client.go`](../pkg/shared/cedar/core/client.go), set `cedarCoreEnabledDefault = true`.
