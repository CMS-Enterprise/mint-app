...
Performing model implementation, design, monitoring, and evaluation activities that include: `,
  ...
  [MtoCommonSolutionKey.CDAC]: {
    about: {
      description: `The CMMI Data Aggregation Contract (CDAC) helps CMMI models by providing comprehensive, multi-source, near real-time insights to model teams and participants to monitor and improve model outcomes. CDAC integrates external data sources (e.g., clinical data, multi-payer claims data) with existing CMS data (e.g., Medicare and Medicaid claims) to provide meaningful and actionable information to CMS and model participants. CDAC is committed to designing and implementing data aggregation strategies that prioritize and address the needs of both model teams and model participants.

  CDAC could support a model/project if:`,
      items: [
        'The model team would benefit from enhanced monitoring capabilities that include the integration of external data sources with internal CMS data sources for more comprehensive monitoring. CDAC has reusable, integrated datasets to get insights to models earlier in the lifecycle for more proactive monitoring and improvement. <italic>For example, CDAC is working with the Cell & Gene Therapy Access (CGT) Model Team to leverage aggregated clinical and claims data (Medicaid, Medicare, and commercial) for tracking longitudinal patient outcomes after a drug therapy – including following patients as they leave Medicaid and transition across care settings and health plans.</italic>',
        `The model participants would benefit from more comprehensive, real-time clinical and claims data insights across care settings and providers (e.g., specialists, hospitals), to improve model outcomes.

        <italic>a. For example, CDAC has enabled participants in primary care models to receive real-time hospital alerts on their patients, allowing prompt and informed care coordination and reducing avoidable readmissions.</italic>
        <italic>b. CDAC has also facilitated delivery of multi-payer gaps in care reports to model participants to improve performance on quality measures.</italic>
        `
      ],
      ordered: true,
      descriptionFooter:
      ...
      components: [
        ...
        },
        {
          ...
          description: `Integrating patient data across time and from various data sources (e.g., clinical registry, commercial payer data), allowing model teams to more timely and comprehensively monitor and improve outcomes.

Examples and Impact:`,
          level: 'h4',
          items: [
            'Longitudinal Patient Tracking: By integrating data from clinical registries, Medicaid, state programs, and commercial insurers, the model teams can monitor patient outcomes over the lifecycle of the model (e.g. for informing outcomes-based payment arrangements with model participants, or validating participant-submitted outcomes with independent clinical data)',
            'Expanded Clinical Data Access: Incorporating clinical data from the HIEs, registries, and other third-party sources allows model teams to track granular clinical outcomes that are not captured in standard CMS claims.',
            'Enhanced Chronic Disease Monitoring: Collecting detailed patient data, such as lab results, enables more effective tracking and management of chronic conditions across patient groups, helping to assess and improve the model’s impact on disease management.'
          ]
        },
        {
          header: 'Integrated data and insights to Model Participants',
          description: `Delivering timely, comprehensive, and actionable multi-source insights to model participants drives improvement on model measures and reduces burden.

Examples and Impact:`,
          level: 'h4',
          items: [
            'Encounter Notification Services: Providers receive real-time alerts about hospital and ambulatory visits on their patients, enabling prompt follow-up and improved care coordination. In PCF, this initiative reduced avoidable readmissions and stays. One practice who received these reports noted they were able to avert 12+ hospital admissions in a single quarter.',
            'Focused Clinical Reports: Practices get lab results reports, such as HbA1c, for their patients. In PCF, over 70% of providers receiving these reports discovered results they did not know about, leading to earlier interventions, less unnecessary testing, and better patient outcomes.',
            'Gaps in Care Reports: Practices receive integrated reports highlighting missing preventative or chronic disease care using data across payers and care settings, allowing providers to target outreach and effectively close care gaps.'
          ]
        },
        {
          header: 'Comprehensive Data Strategy',
          description: `Develop an efficient approach to identifying, acquiring, integrating, and using data sources for actionable analytics across the model lifecycle.

Examples and Impact:`,
          level: 'h4',
          items: [
            'Plan to save time and effort on data acquisition and integration: CDAC can identify ways to most efficiently use existing CDAC solutions and source additional datasets unique to your model needs. Our proven market assessment and capability readiness frameworks help model teams select the best data sources for use by CMS teams and by model participants. Planning a comprehensive data strategy early on helps avoid scrambling for information down the line, and takes the burden of data acquisition and management off the model team.'
          ]
        }
      ]
    },
