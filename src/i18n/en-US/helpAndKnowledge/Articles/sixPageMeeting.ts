const sixPageMeeting = {
  ...
  additionalResources: {
    heading: 'Additional resources available to model teams',
    description:
      'Model teams may have already spoken with CMMI FO about potential contractor support during the 2-page review meeting. If not, they should generally have a better idea of the support they will need during the model concept and design phase since the 6-page concept paper should also address contractor support. ',
    pipelineContractors: {
      heading: 'Pipeline contractors',
      description: `CMMI generally has two pipeline contractors available (currently MITRE and ARDS have pipeline contracts with CMMI) that can assist model teams with various needs leading up to clearance. It’s important to note, model teams need to get approval from CMMI FO to utilize one of these contractors, so it can be important for model teams to begin considering contractor involvement even as early as the 2-page concept phase.
      
  While it is fine to use pipeline support for 2-page and 6-page concept paper work and for developing Innovation Center Investment Proposal (ICIP) content, after the ICIP clears, model teams can no longer use pipeline funding. It can take a few months to get implementation contractor support, so plan accordingly. Model teams should work with the Division of Central Contracts Services (DCCS), the Division of Systems Support, Operation & Security (DSSOS), and the Office of Acquisition and Grants Management (OAGM) for assistance with contracting needs.`
    },
    tipsSummaryBox: {
      heading: 'Tips for model teams',
      description:
        'Discuss the following questions with your team to determine whether you could benefit from utilizing a pipeline contractor:',
      items: [
