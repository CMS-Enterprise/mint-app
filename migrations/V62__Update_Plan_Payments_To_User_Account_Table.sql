...
WITH userAccount AS (
    ...
    FROM plan_payments
    LEFT JOIN user_account AS user_account_ready_for_clearance ON plan_payments.ready_for_clearance_by_old = user_account_ready_for_clearance.username
    LEFT JOIN user_account AS user_account_ready_for_review ON plan_payments.ready_for_review_by_old = user_account_ready_for_review.username
    LEFT JOIN user_account AS user_account_created ON plan_payments.created_by_old = user_account_created.username
    LEFT JOIN user_account AS user_account_modified ON plan_payments.modified_by_old = user_account_modified.username
)
--
...
FROM userAccount
WHERE userAccount.primaryID = plan_payments.id;
