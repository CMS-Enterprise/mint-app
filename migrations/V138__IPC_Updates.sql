...
WITH poc_solkeys_to_remove(SolutionKey) AS (VALUES('IPC')),
     pocs_to_remove(POCName) AS (VALUES('Sue Nonemaker'), ('Philip Tennant'))
...
WHERE possible_operational_solution_id IN (
  ...
  WHERE CAST(sol_key as TEXT) IN (SELECT SolutionKey FROM poc_solkeys_to_remove)
)
  AND name = ANY(SELECT POCName FROM pocs_to_remove);

/* Set up new POCs */
WITH pocs(SolutionName, SolutionKey, Name, Email, Role, IsTeam) AS (VALUES
                                                                      ('Innovation Payment Contractor (IPC)', 'IPC', 'Alistair Quatre', 'alistair.quatre@cms.hhs.gov', 'Subject Matter Expert', FALSE),
                                                                      ('Innovation Payment Contractor (IPC)', 'IPC', 'Brandon Cooper', 'brandon.cooper@cms.hhs.gov', 'Subject Matter Expert', FALSE)
)
