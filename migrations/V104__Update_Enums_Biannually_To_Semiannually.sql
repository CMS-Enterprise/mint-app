BEGIN;
  ...
  SET data_sharing_frequency = replace_element(data_sharing_frequency, 'BIANNUALLY', 'SEMIANNUALLY')
  WHERE 'BIANNUALLY' = ANY(data_sharing_frequency);

  UPDATE plan_ops_eval_and_learning
  SET data_collection_frequency = replace_element(data_collection_frequency, 'BIANNUALLY', 'SEMIANNUALLY')
  WHERE 'BIANNUALLY' = ANY(data_collection_frequency);

  UPDATE plan_payments
  SET anticipated_payment_frequency = replace_element(anticipated_payment_frequency, 'BIANNUALLY', 'SEMIANNUALLY')
