...
WITH userAccount AS (
    SELECT
        plan_ops_eval_and_learning.id AS primaryID,
    ...
    LEFT JOIN user_account AS user_account_ready_for_clearance ON plan_ops_eval_and_learning.ready_for_clearance_by_old = user_account_ready_for_clearance.username
    LEFT JOIN user_account AS user_account_ready_for_review ON plan_ops_eval_and_learning.ready_for_review_by_old = user_account_ready_for_review.username
    LEFT JOIN user_account AS user_account_created ON plan_ops_eval_and_learning.created_by_old = user_account_created.username
    LEFT JOIN user_account AS user_account_modified ON plan_ops_eval_and_learning.modified_by_old = user_account_modified.username
)
--
...
FROM userAccount
WHERE userAccount.primaryID = plan_ops_eval_and_learning.id;
...
WHERE name = 'plan_ops_eval_and_learning';
